...
async function handleDownloadTemplate() {
  try {
    await alumniStore.downloadTemplate()
  } catch {
...
async function handleUpload(file) {
  ...
  try {
    ...
    toast[result.failed > 0 ? 'warning' : 'success'](`Import selesai: ${result.success} berhasil, ${result.failed} gagal.`)
  } catch {
    toast.error('Import gagal. Periksa format file Anda.')
  } finally {
    ...
    <div class="max-w-2xl space-y-6">
      <div class="card p-6">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <button class="mt-3 btn-secondary flex items-center gap-2" @click="handleDownloadTemplate">
      ...
      <div v-if="uploading" class="card p-6">
        ...
        <div class="w-full bg-gray-200 rounded-full h-2">
          <div class="bg-primary-600 h-2 rounded-full transition-all duration-300" :style="{ width: `${progress}%` }" />
        </div>
      ...
      <div v-if="importResult" class="card p-6">
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
          <div class="text-center p-4 bg-green-50 rounded-lg"><p class="text-2xl font-bold text-green-700">{{ importResult.success }}</p><p class="text-sm text-green-600">Berhasil</p></div>
          <div class="text-center p-4 bg-red-50 rounded-lg"><p class="text-2xl font-bold text-red-700">{{ importResult.failed }}</p><p class="text-sm text-red-600">Gagal</p></div>
          <div class="text-center p-4 bg-yellow-50 rounded-lg"><p class="text-2xl font-bold text-yellow-700">{{ importResult.skipped ?? 0 }}</p><p class="text-sm text-yellow-600">Dilewati (Duplikat)</p></div>
        </div>
        ...
        <div v-if="importResult.errors?.length" class="mt-4">
          ...
          <div class="max-h-48 overflow-y-auto border border-red-200 rounded-lg">
            <table class="w-full text-xs">
              ...
              <tbody>
                <tr v-for="err in importResult.errors" :key="err.row" class="border-t border-red-100">
