...
// String (bukan Array) — FileUpload.accept bertipe String dan memanggil .split(',')
...
<template>
  <div>
    ...
    <div class="flex items-center gap-3 mb-6">
      <button class="text-gray-500 hover:text-gray-700 p-1 rounded" @click="goBack" aria-label="Kembali">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
        </svg>
      </button>
      <div>
        <h1 class="text-xl font-semibold text-gray-900">Import Alumni</h1>
      ...
      <div class="card p-6">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-gray-500 mt-1">Format .xlsx atau .xls, maksimal 10 MB.</p>
            <div class="mt-3">
              <FileUpload
                :accept="ACCEPT_EXTENSIONS"
                :max-size-mb="10"
                label="Upload File Excel"
                hint="Format .xlsx atau .xls · Maks. 10 MB"
                :disabled="uploading"
      ...
      <div v-if="importResult" class="card p-6">
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
          ...
          <div class="text-center p-4 bg-yellow-50 rounded-lg">
            ...
            <p class="text-sm text-yellow-600">Duplikat</p>
          </div>
        </div>
        ...
        <div class="flex gap-3 mt-5">
          ...
          <button class="btn-secondary" @click="resetImport">Import Lagi</button>
        </div>
      </div>
    </div>
  </div>
...
<style scoped>
.btn-primary  { @apply bg-teal-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors; }
...
.step-badge  { @apply w-8 h-8 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-sm flex-shrink-0; }
</style>
