...
async function fetchYears() {
  ...
  try {
    ...
    showToast('Gagal memuat data tahun wisuda.', 'error')
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    ...
    if (editMode.value) {
      ...
      showToast('Tahun wisuda berhasil diperbarui.', 'success')
    } else {
      const { data } = await api.post('/admin/graduation-years', payload)
      years.value.push(data.data)
      showToast('Tahun wisuda berhasil ditambahkan.', 'success')
    }
    closeModal()
  } catch (err) {
...
async function destroy(year) {
  ...
  try {
    ...
    showToast('Tahun wisuda berhasil dihapus.', 'success')
  } catch (err) {
