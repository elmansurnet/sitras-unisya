<template>
  <router-link
    ...
    :aria-label="label"
  >
    <!-- Icon (Lucide via slot atau nama string) -->
    <span v-if="icon" class="w-4 h-4 flex-shrink-0" aria-hidden="true">
      <component :is="iconComponents[icon]" v-if="iconComponents[icon]" />
      <svg v-else class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        <circle cx="12" cy="12" r="2" />
      </svg>
    </span>
    <!-- Titik kecil untuk child item tanpa icon -->
...
// Inline SVG icon map (subset Lucide — hanya yang dipakai di sidebar)
const iconComponents = {
  'grid': {
    template: `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
  ...
  'user-cog': {
    template: `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <path stroke-linecap="round" stroke-linejoin="round" d="M3 21v-2a4 4 0 014-4h4"/>
      <circle cx="19" cy="19" r="2"/>
      <path stroke-linecap="round" stroke-linejoin="round" d="M19 15v2M19 21v2M15.34 16.34l1.41 1.41M21.25 21.25l1.41 1.41M13.75 21.25l-1.41 1.41M19.25 16.34l-1.41 1.41"/>
    </svg>`,
  },
  'settings': {
    template: `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
